...
function Suggestion($suggestion,$user_id)
{?>
<html>
<head>
<style>
  ...
  ?><center>
    <div class="column6">
      ...
      <form action="" method="POST">
        <label for="user_name">User Id</label><br><font size=2 color=red>User Id is not mandatory</font><br>
        <input type="text"   name="user_id" value="<?php echo $user_id; ?>"placeholder="User Id"/><br>
        <label for="suggestion" required>Write Suggestion here</label><br>
        <textarea rows="8" cols="50" name="suggestion" value="<?php echo $suggestion; ?>" required></textarea><br>
...
<?php
  ...
  { 
    ...
    $user_id = $_POST['user_id'];
    if($user_id == '')
      $sql=mysqli_query($conn,"INSERT INTO complaints (Suggestion) VALUES ('".$suggestion."')");
    else
      $sql=mysqli_query($conn,"INSERT INTO complaints (Suggestion, User_id) VALUES ('".$suggestion."',".$user_id.")");
    //echo "INSERT INTO complaints (Suggestion, User_id) VALUES ('".$suggestion."',".$user_id.")";
    if($sql)
    {
      $sql2=mysqli_query($conn,"SELECT Name FROM users WHERE Enrol_id=".$user_id);
      $row=mysqli_num_rows($sql2);
      while($row = mysqli_fetch_array($sql2))
        $name=$row['Name'];
      echo "<script>alert('A Suggestion has been sent')</script>";
      echo "<script>window.location = 'suggestions.php'</script>";
    }
    else
    {
